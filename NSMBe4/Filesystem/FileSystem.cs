...
namespace NSMBe4.Filesystem
{
    public abstract class FileSystem
    {
        ...
        private Dictionary<ushort, File> files;
        ...
        protected void ResetDictionaries()
        {
            ...
            if (FileIDs == null)
            {
                ...
                files = new Dictionary<ushort, File>();
            }
            FileIDs.Clear();
            FileNames.Clear();
            FileOffsets.Clear();
        ...
        protected void createFileInstances()
        {
            foreach (ushort u in FileIDs.Values)
                files[u] = new File(u, this);
        }

        public File getFile(string name)
        {
            return files[FileIDs[name]];
        }

        public File getFile(ushort id)
        {
            return files[id];
        }

        public FileReference openFile(string name, bool readOnly)
        {
            File f = files[FileIDs[name]];
            return f.OpenFile(readOnly);
        }

        public FileReference openFile(ushort id, bool readOnly)
        {
            File f = files[id];
            return f.OpenFile(readOnly);
        }
